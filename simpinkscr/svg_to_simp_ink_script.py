...
class SvgToPythonScript(inkex.OutputExtension):
    ...
    _common_svg_defaults = {"stroke": repr("none"), "fill": repr("black")}

    # Most Simple Inkscape Scripting shapes use this as their default style.
    _common_sis_defaults = {
        "stroke": "#000000",  # More common than "black"
        "fill": "none",
    }
    ...
    def style_args(self, node, def_svg_style, def_sis_style):
        ...
        if node.get("sis_within_marker") == "true":
            def_svg_style = {"stroke": None, "fill": None}
    ...
    def extra_args(self, node, def_svg_style=None, def_sis_style=None):
        ...
        style_args, s_deps = self.style_args(node, def_svg_style, def_sis_style)
        args = [
            self.transform_arg(node),
    ...
    def convert_ellipse(self, node):
        ...
        extra, extra_deps = self.extra_args(node)
        code = ["ellipse((%.10g, %.10g), (%.10g, %.10g)%s)" % (cx, cy, rx, ry, extra)]
        return self.Statement(code, node.get_id(), extra_deps)
    ...
    def convert_rectangle(self, node):
        ...
        code = [
            "rect((%.10g, %.10g), (%.10g, %.10g)%s)" % (x, y, x + wd, y + ht, extra)
        ]
        return self.Statement(code, node.get_id(), extra_deps)
    ...
    def convert_line(self, node):
        ...
        extra, extra_deps = self.extra_args(node)
        code = ["line((%.10g, %.10g), (%.10g, %.10g)%s)" % (x1, y1, x2, y2, extra)]
        return self.Statement(code, node.get_id(), extra_deps)
    ...
    def convert_poly_star(self, node):
        ...
        if flat == "true":
            # Regular polygon
            code = [
                "regular_polygon(%s, (%s, %s), %s, angles=%s%s%s)"
                % (sides, cx, cy, r1, arg1, opt_arg_str, extra)
            ]
        else:
            # Star
            code = [
                "star(%s, (%s, %s), (%s, %s), angles=(%s, %s)%s%s)"
    ...
    def convert_connector(self, node):
        ...
        code = ["connector(%s, %s%s%s)" % (var1, var2, opt_arg_str, extra)]
    ...
    def convert_path(self, node):
        ...
        if len(pe_list) == 1:
            ...
            pe_list_str = ", ".join([self.Statement.id2var(pe) for pe in pe_list])
            code.append(
                "%s.apply_path_effect([%s])"
                % (self.Statement.id2var(node.get_id()), pe_list_str)
            )
    ...
    def convert_text(self, node):
        ...
        tpaths = [
            c.get("xlink:href")[1:] for c in node.iter() if c.tag[-8:] == "textPath"
        ]
        if tpaths != []:
        ...
        if msg is None:
            msg = ""
        extra, extra_deps = self.extra_args(node, {}, {})
        code = ["text(%s, (%.10g, %.10g)%s%s)" % (repr(msg), x, y, tpath_str, extra)]
        ...
        for tspan in [c for c in node.iter() if c.tag[-5:] == "tspan"]:
            if tspan.text is not None:
                ...
                # and style.
                need_var_name = True
                x = self.number_to_pixels(tspan.get("x"), pct_of="wd", default=0)
                y = self.number_to_pixels(tspan.get("y"), pct_of="ht", default=0)
                extra, extra_deps = self.extra_args(tspan, {})
                all_deps = all_deps.union(extra_deps)
                if x is not None and y is not None:
                    # Specified position
                    code.append(
                        "%s.add_text(%s, (%.10g, %.10g)%s)"
                        % (var_name, repr(tspan.text), x, y, extra)
                    )
                else:
                    # Unspecified position
                    code.append(
                        "%s.add_text(%s%s)" % (var_name, repr(tspan.text), extra)
                    )
    ...
    def convert_image(self, node):
        ...
        if href is not None and href[:5] == "data:":
            ...
            code = ["image(%s, (%s, %s), embed=False%s)" % (repr(href), x, y, extra)]
        elif absref is not None:
            # Non-embedded image.  We were given the original filename.
            code = ["image(%s, (%s, %s), embed=False%s)" % (repr(absref), x, y, extra)]
        else:
    ...
    def convert_foreign(self, node):
        ...
        code = [
            "foreign((%.10g, %.10g), (%.10g, %.10g)%s)" % (x, y, x + wd, y + ht, extra)
        ]
        return self.Statement(code, node.get_id(), extra_deps)
    ...
    def _common_gradient_args(self, node):
        ...
        if spread is not None and spread != "pad":
            spread_to_repeat = {"reflect": "reflected", "repeat": "direct"}
    ...
    def _gradient_stops(self, node):
        ...
        for stop in node:
            ...
            code.append(
                "%s.add_stop(%s%s)" % (var_name, ", ".join(stop_args), style_str)
            )
        return code, deps
    ...
    def convert_marker(self, node):
        ...
        if v_box is not None:
            ...
            marker_args.append(
                "view_box=((%.10g, %.10g), (%.10g, %.10g))" % (x0, y0, x0 + wd, y0 + ht)
            )
    ...
    def convert_hyperlink(self, node):
        ...
        code = ["hyperlink(%s, %s%s)" % (child_vars_str, link_args_str, extra)]
        return self.Statement(code, node.get_id(), child_ids + extra_deps)
    ...
    def convert_guide(self, node):
        ...
        # Generate code and wrap it in a statement.
        code = [
            "guides.append(guide((%.10g, %.10g), %.10g%s))"
        ...
        for node in self.svg.xpath(known_tags):
            if isinstance(node, inkex.Circle):
                ...
                raise inkex.AbortExtension(
                    _("Internal error converting %s" % repr(node))
                )
